...
void yolov2_forward_network_cpu(network net, network_state state) {
    ...
    for (i = 0; i < net.n; ++i) {
        ...
        printf("%d;%ld", l.outputs, timeElapsed / (CLOCKS_PER_SEC / 1000));
        int run = 0;
        ...
        /*
         ...
         zfp_field *field = zfp_field_1d(&state.input[0], type, l.outputs);
        ...
        uchar *buffer =  malloc(bufsize);

        bitstream *stream = stream_open(buffer, bufsize);
        zfp_stream_set_accuracy(zfp, 0.00000009);       // bit stream to compress to
...
// compress array
        ...
        printf("\n Compressed \t\t;%d ", zfpsize);
        printf("\n Raw ;%d ;%d \n", i, l.outputs);
*/

        // FILE* file = fopen("rawData", "wb");
        //fwrite(buffer, sizeof(*buffer), l.outputs, filePtr);
        printf("first:  %lf\n", state.input[0]);
        printf("last %lf \n", state.input[l.outputs - 1]);
        fwrite(state.input, sizeof(*state.input),l.outputs, filePtr);
        fclose(filePtr);
        /*
        ...
        float *buffer;
        buffer = (float *) malloc(sizeof(*state.input) * l.outputs);
        fread(buffer, 1, l.outputs*sizeof(*state.input), filePtr);
        for (int i = 0; i < l.outputs; i++) {
            if (buffer[i] != state.input[i]) {
                printf("Huch %d", i);
                exit(0);
            }
        }
